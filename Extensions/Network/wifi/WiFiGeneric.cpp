...
class_WiFiGeneric::class_WiFiGeneric() {
...
// ToDo: From here is block of event-messaging system. need to recode it and move to main System to initiate events from device
...
static void _arduino_event_task(void * arg){
    ...
    for (;;) {
        if(xQueueReceive(_arduino_event_queue, &data, portMAX_DELAY) == pdTRUE){
            class_WiFiGeneric::_eventCallback(data);
            free(data);
            data = NULL;
        }
    }
...
// Function initiates event queue. next it goes to _arduino_event_task. typicaly it is freeRTOS, not arduino feature.
...
bool class_WiFiGeneric::_wifiUseStaticBuffers = false;

bool class_WiFiGeneric::useStaticBuffers(){
    return _wifiUseStaticBuffers;
}

void class_WiFiGeneric::useStaticBuffers(bool bufferMode){
    if (lowLevelInitDone) {
...
bool wifiLowLevelInit(bool persistent){
	...
	if(!class_WiFiGeneric::useStaticBuffers()) {
...
bool class_WiFiGeneric::_persistent = true;
bool class_WiFiGeneric::_long_range = false;
wifi_mode_t class_WiFiGeneric::_forceSleepLastMode = WIFI_MODE_NULL;
#if CONFIG_IDF_TARGET_ESP32S2
wifi_ps_type_t class_WiFiGeneric::_sleepEnabled = WIFI_PS_NONE;
#else
wifi_ps_type_t class_WiFiGeneric::_sleepEnabled = WIFI_PS_MIN_MODEM;
#endif
...
/* moved to Host;
const char * class_WiFiGeneric::getHostname()
{
    return get_esp_netif_hostname();
}

bool class_WiFiGeneric::setHostname(const char * hostname)
{
...
int class_WiFiGeneric::setStatusBits(int bits){
    if(!_arduino_event_group){
        return 0;
    }
...
int class_WiFiGeneric::clearStatusBits(int bits){
    if(!_arduino_event_group){
        return 0;
    }
...
int class_WiFiGeneric::getStatusBits(){
    if(!_arduino_event_group){
        return 0;
    }
...
int class_WiFiGeneric::waitStatusBits(int bits, uint32_t timeout_ms){
    if(!_arduino_event_group){
        return 0;
    }
 ...
 */
wifi_event_id_t class_WiFiGeneric::onEvent(WiFiEventCb cbEvent, arduino_event_id_t event)
{
    if(!cbEvent) {
        return 0;
    }
...
wifi_event_id_t class_WiFiGeneric::onEvent(WiFiEventFuncCb cbEvent, arduino_event_id_t event)
{
    if(!cbEvent) {
        return 0;
    }
...
wifi_event_id_t class_WiFiGeneric::onEvent(WiFiEventSysCb cbEvent, arduino_event_id_t event)
{
    if(!cbEvent) {
        return 0;
    }
 ...
 * @param event optional filter (WIFI_EVENT_MAX is all events)
 */
void class_WiFiGeneric::removeEvent(WiFiEventCb cbEvent, arduino_event_id_t event)
{
    if(!cbEvent) {
        return;
    }
...
void class_WiFiGeneric::removeEvent(WiFiEventSysCb cbEvent, arduino_event_id_t event)
{
    if(!cbEvent) {
        return;
    }
...
void class_WiFiGeneric::removeEvent(wifi_event_id_t id)
{
    for(uint32_t i = 0; i < cbEventList.size(); i++) {
        WiFiEventCbList_t entry = cbEventList[i];
        if(entry.id == id) {
...
#endif
esp_err_t class_WiFiGeneric::_eventCallback(arduino_event_t *event)
{
    ...
    // ToDo: move to Ethernet from here (below)
    ...
    // ToDo: move to Ethernet to here (above)

    // ToDo: Next move to IPv4/IPv6(if will be implemented)
 ...
 * @return channel (1-13)
 */
int32_t class_WiFiGeneric::channel(void)
{
 ...
 * @param persistent
 */
void class_WiFiGeneric::persistent(bool persistent)
{
 ...
 * @param enable
 */
void class_WiFiGeneric::enableLongRange(bool enable)
{
    _long_range = enable;
}
 ...
 * @param m WiFiMode_t
 */
bool class_WiFiGeneric::mode(wifi_mode_t m)
{
 ...
 * @return WiFiMode
 */
wifi_mode_t class_WiFiGeneric::getMode()
{
 ...
 * @return ok
 */
bool class_WiFiGeneric::enableSTA(bool enable)
{
 ...
 * @return ok
 */
bool class_WiFiGeneric::enableAP(bool enable)
{
 ...
 * @return ok
 */
bool class_WiFiGeneric::setSleep(bool enabled){
 ...
 * @return ok
 */
bool class_WiFiGeneric::setSleep(wifi_ps_type_t sleepType)
{
 ...
 */
wifi_ps_type_t class_WiFiGeneric::getSleep()
{
 ...
 * @return ok
 */
bool class_WiFiGeneric::setTxPower(wifi_power_t power){
...
wifi_power_t class_WiFiGeneric::getTxPower(){
    int8_t power;
    if((getStatusBits() & (STA_STARTED_BIT | AP_STARTED_BIT)) == 0){
        log_w("Neither AP or STA has been started");
 ...
 */
bool class_WiFiGeneric::initiateFTM(uint8_t frm_count, uint16_t burst_period, uint8_t channel, const uint8_t * mac) {
 ...
 * @return true on success
 */
bool class_WiFiGeneric::setDualAntennaConfig(uint8_t gpio_ant1, uint8_t gpio_ant2, wifi_rx_ant_t rx_mode, wifi_tx_ant_t tx_mode) {
...
/* moved to Network/IPv4

IPAddress class_WiFiGeneric::calculateNetworkID(IPAddress ip, IPAddress subnet) {
	IPAddress networkID;
...
IPAddress class_WiFiGeneric::calculateBroadcast(IPAddress ip, IPAddress subnet) {
...
uint8_t class_WiFiGeneric::calculateSubnetCIDR(IPAddress subnetMask) {
	uint8_t CIDR = 0;
